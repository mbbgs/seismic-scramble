...
export class GameManager {
  ...
  async startGame() {
    try {
      ...
      if (response.success) {
        this.gameHash = response.data.hash_id;
  ...
  startTimer() {
    this.timer = setInterval(() => {
      ...
      if (elapsed >= this.maxGameTime) {
        this.endGame(true); // true = timeout
      }
      
      // Update UI timer if callback exists
  ...
  async submitScore(finalScore) {
    if (!this.isGameActive || !this.gameHash) {
      return {
        success: false,
    ...
    try {
      const response = await API.submitScore(this.gameHash, finalScore);
...
export class UserProfileManager {
  ...
  displayProfile(profile, containerElement) {
    if (!containerElement || !profile) return;
    
    containerElement.innerHTML = `
      <div class="flex items-center gap-4">
        <img src="/default-avatar.jpg" 
             class="w-16 h-16 rounded-full border-2 border-gray-900 shadow-glow grayscale" 
             alt="${profile.username}">
        <div>
          <h3 class="text-xl font-bold text-gray-900">${this.escapeHtml(profile.username)}</h3>
          <p class="text-gray-600">Score: ${profile.score} | Rank: #${profile.rank}</p>
        </div>
      </div>
    `;
  }
